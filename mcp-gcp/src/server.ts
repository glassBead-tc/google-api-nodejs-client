...
async function getAuth(scopes: string[]): Promise<AuthClient> {
  const auth = new GoogleAuth({ scopes });
  return auth.getClient();
}

async function getProject(scopes: string[]): Promise<string> {
  const auth = new GoogleAuth({ scopes });
  const projectFromEnv = getDefaultProjectId();
  if (projectFromEnv) return projectFromEnv;
  const project = await auth.getProjectId();
  if (!project) throw new Error('No default project found. Set GOOGLE_CLOUD_PROJECT or GCLOUD_PROJECT.');
  return project;
}
...
// gcp.whoami
{
  const args = { includeScopes: z.boolean().optional() } as const;
  mcp.tool('gcp.whoami', 'Show current ADC identity and default project.', args, async (_args) => {
    ...
    const scopes = (client as any).scopes ?? undefined;
    return {
...
// gcp.projects.list
{
  ...
  mcp.tool('gcp.projects.list', 'List projects using Cloud Resource Manager v3.', args, async (input) => {
    ...
    const res = await crm.projects.list({ pageSize: input.pageSize, pageToken: input.pageToken } as any);
    return { content: [{ type: 'text', text: jsonStringify((res as any).data) }] };
  });
}

// gcs.buckets.list
{
  const args = { projectId: z.string().optional() } as const;
  mcp.tool('gcs.buckets.list', 'List GCS buckets in a project.', args, async (input) => {
    ...
    const res = await storage.buckets.list({ project } as any);
    return { content: [{ type: 'text', text: jsonStringify((res as any).data) }] };
  });
}
...
// gcs.objects.list
{
  ...
  mcp.tool('gcs.objects.list', 'List objects in a GCS bucket.', args, async (input) => {
    ...
    google.options({ auth: authClient as any });
    const storage = google.storage('v1');
    const res = await storage.objects.list({ bucket: input.bucket, prefix: input.prefix, maxResults: input.maxResults } as any);
    return { content: [{ type: 'text', text: jsonStringify((res as any).data) }] };
  });
}
...
{
  const args = { bucket: z.string(), object: z.string(), generation: z.string().optional() } as const;
  mcp.tool('gcs.objects.download', 'Download a small text object from GCS (returns content inline).', args, async (input) => {
    const authClient = await getAuth(['https://www.googleapis.com/auth/devstorage.read_only']);
    google.options({ auth: authClient as any });
    const storage = google.storage('v1');
    const res = await storage.objects.get({ bucket: input.bucket, object: input.object, generation: input.generation, alt: 'media' as any } as any);
    const data = ((res as any).data as unknown) as string | Buffer | object;
    let text: string;
    if (typeof data === 'string') text = data;
    else if (Buffer.isBuffer(data)) text = data.toString('utf-8');
    else text = jsonStringify(data);
...
{
  const args = { projectId: z.string().optional(), secretId: z.string(), version: z.string().optional().default('latest') } as const;
  mcp.tool('secretmanager.secrets.access', 'Access a Secret Manager version payload as text (UTF-8).', args, async (input) => {
    ...
    const res = await sm.projects.secrets.versions.access({ name } as any);
    const dataAny = (res as any).data as any;
    const payload = (dataAny.payload && dataAny.payload.data) ? Buffer.from(dataAny.payload.data as string, 'base64').toString('utf-8') : '';
...
// pubsub.topics.list
{
  const args = { projectId: z.string().optional(), pageSize: z.number().int().min(1).max(1000).optional(), pageToken: z.string().optional() } as const;
  mcp.tool('pubsub.topics.list', 'List Pub/Sub topics in a project.', args, async (input) => {
    ...
    const res = await pubsub.projects.topics.list({ project: `projects/${project}`, pageSize: input.pageSize, pageToken: input.pageToken } as any);
    return { content: [{ type: 'text', text: jsonStringify((res as any).data) }] };
  });
}
...
// run.services.list
{
  const args = { projectId: z.string().optional(), region: z.string().optional().default('us-central1') } as const;
  mcp.tool('run.services.list', 'List Cloud Run services in a region.', args, async (input) => {
    ...
    const res = await run.projects.locations.services.list({ parent } as any);
    return { content: [{ type: 'text', text: jsonStringify((res as any).data) }] };
  });
}
...
{
  const args = { projectId: z.string().optional(), zone: z.string().optional() } as const;
  mcp.tool('compute.instances.list', 'List Compute Engine instances. If zone omitted, lists aggregated.', args, async (input) => {
    ...
    if (input.zone) {
      const res = await compute.instances.list({ project, zone: input.zone } as any);
      return { content: [{ type: 'text', text: jsonStringify((res as any).data) }] };
    }
    const res = await compute.instances.aggregatedList({ project } as any);
    return { content: [{ type: 'text', text: jsonStringify((res as any).data) }] };
  });
}
...
// gapi.request (generic)
{
  ...
  mcp.tool('gapi.request', 'Generic Google API call via discovery clients.', args, async (input) => {
    ...
    google.options({ auth: authClient as any });
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const service: any = (google as unknown as GoogleApis)[input.api]?.call(google, input.version);
    if (!service) throw new Error(`Unknown API ${input.api} ${input.version}`);
    const segments = input.method.split('.');
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let target: any = service;
    for (let i = 0; i < segments.length - 1; i++) {
      target = target[segments[i]];
      if (!target) throw new Error(`Invalid method path at ${segments.slice(0, i + 1).join('.')}`);
    }
    const fn = target[segments[segments.length - 1]];
    if (typeof fn !== 'function') throw new Error('Final method is not callable');
    const res = await fn({ ...(input.parameters ?? {}) } as any);
    return { content: [{ type: 'text', text: jsonStringify((res as any).data) }] };
  });
}
